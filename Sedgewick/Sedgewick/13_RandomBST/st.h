...
//В рандомизированном BST-дереве каждый из узлов с равной вероятностью является корнем
//Для конструирования рандомного BST-дерева из N элементов используется около 2NlnN сравнений
...
private:
    ...
    link joinLR(link a, link b)
    {
        ...
        { a->r = joinLR(a->r, b); a->upd(); return a; }
        else{ b->l = joinLR(a, b->l); b->upd(); return b; }
    }
    ...
    void insertR(link& h, Item x)
    {
        ...
        if(rand() < RAND_MAX/(h->N + 1)){ insertT(h, x); return; }
...
public:
    ...
    void insert(Item x){ insertR(head, x); head->upd(); }
    void remove(Item x){ removeR(head, x.key()); head->upd(); }
    void join(ST<Key, Item>& b)
    {
        int N = head->N;
        if(rand()*(RAND_MAX/(N + b.head->N) + 1) < N) head = joinR(head, b.head);
