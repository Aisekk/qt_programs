...
//-------------------------------------------------------------------------------------------------
//save wafer params
void ConfigWin::saveWaferSqlTableModel(size_t slotNum, shared_ptr<QSqlTableModel> spSqlTableModel)
{
    m_map_spSqlWaferTableModel.insert(slotNum, spSqlTableModel);
}
void ConfigWin::saveWaferTableView(size_t slotNum, shared_ptr<QTableView> spTableView)
{
    m_map_spWaferTableView.insert(slotNum, spTableView);
}
//save wafer values
void ConfigWin::saveWaferValuesSqlTableModel(size_t slotNum,
                                             shared_ptr<QSqlTableModel> spSqlTableModel)
{
    m_map_spSqlTestElemValuesTableModel.insert(slotNum, spSqlTableModel);
}
void ConfigWin::saveWaferValuesTableView(size_t slotNum,
                                         shared_ptr<QTableView> spTableView)
{
    m_map_spTestElemValuesTableView.insert(slotNum, spTableView);
}

//save lot data
void ConfigWin::saveLotDataSqlTableModel(size_t slotNum,
                                         shared_ptr<QSqlTableModel> spSqlTableModel)
{
    m_map_spSqlLotDataTableModel.insert(slotNum, spSqlTableModel);
}
void ConfigWin::saveLotDataTableView(size_t slotNum,
                                     shared_ptr<QTableView> spTableView)
{
...
void ConfigWin::slotWaferParamsTableWin()
{
    ...
    if(slotNum > 0)
    {
        QString WaferParamsTable =
                DataBase::WaferParamsSqlTable  + QString::number(slotNum,10);
        //-----------------------------------------------------------------------------------------
        //создать модель
...
void ConfigWin::slotWaferValuesTableWin()
{
    ...
    if(slotNum > 0)
    {
        QString WaferValuesTable =
                DataBase::WaferValuesSqlTable + QString::number(slotNum,10);
        //-----------------------------------------------------------------------------------------
        //создать модель
        shared_ptr<QSqlTableModel> spSqlTableModel = make_shared<QSqlTableModel>();
        if(spSqlTableModel)
...
void ConfigWin::slotLotTableWin()
{
    ...
    if(slotNum > 0)
    {
        QString LotDataTable = DataBase::LotDataSqlTable;// + QString::number(slotNum,10);
        //-----------------------------------------------------------------------------------------
        //создать модель
        shared_ptr<QSqlTableModel> spSqlTableModel = make_shared<QSqlTableModel>();
        if(spSqlTableModel)
