...
//===============================================================================================
MainWindow::MainWindow(QWidget* pwgt) : QMainWindow(pwgt)//, ui(new Ui::MainWindow)
{
    m_upImpl.reset(new Impl);
    //---------------------------------------------------------------------------------------------
...
void MainWindow::closeEvent(QCloseEvent* pe)
{
    ...
    if(SystemDataBase::pBreaker != nullptr)
    {
        ...
        {
            SystemDataBase::pBreaker->slotBreakCalc();
            pe->accept();
            break;
        }
        case QMessageBox::No:
    ...
    //-------------------------------------------------------------------
    SystemDataBase::upSavingWin = nullptr;
}
//-----------------------------------------------------------------------
...
void MainWindow::createActions()
{
    ...
    connect(m_pactOpen, QAction::triggered, this, slotFileLoad_4082);   
    ...
    m_pactUpdateDataBase = new QAction("Обновить базу данных партии", nullptr);
    m_pactUpdateDataBase->setText("&Обновить базу данных партии ...");
    m_pactUpdateDataBase->setToolTip("Обновить базу данных партии");
    m_pactUpdateDataBase->setIcon(QPixmap(":/db_update.png"));
    connect(m_pactUpdateDataBase, QAction::triggered, this, slotUpdateDatabase);
    m_pactUpdateDataBase->setVisible(false);
    //---------------------------------------------------------------------------------------------
}
...
void MainWindow::createNormsActions()
{
    ...
    connect(m_pactZshNorms035, QAction::triggered, this, slotShowZashita);
    ...
    connect(m_pactFauNorms035, QAction::triggered, this, slotShowFaundri);
    ...
    //переключиться на локальные интервалы пользователя
...
{  
    ...
    m_pmnuFile->addAction(m_pactUpdateDataBase);
    m_pmnuFile->addSeparator();
...
void MainWindow::createToolBars()
{
    ...
    m_pToolBar->addAction(m_pactUpdateDataBase);
    m_pToolBar->addSeparator();
...
void MainWindow::slotShowZashita(){statusBar()->showMessage("Интервалы: ZSH",0);}
void MainWindow::slotShowFaundri(){statusBar()->showMessage("Интервалы: FAU",0);}
...
void MainWindow::slotFileLoad_4082()
{
    ...
    Parsing parsing_switch = Parsing::on;
...
void MainWindow::slotFileLoad_ResMap()
{
    ...
    case Parsing::on:
    {
        ...
        for(;cit != strFiles.constEnd(); ++cit)
        {
            ...
            ParserResMap parser;
            parser.doParsing(ReadFileResMap);
...
void MainWindow::createTreeViewConnections(const TreeView* pTreeView) const
{
    ...
    connect(pTreeView, TreeView::interval, this, slotTestElemIntervalWin);
}
//-------------------------------------------------------------------------------------------------
//настроить интервал тестового элемента
void MainWindow::slotTestElemIntervalWin()
{
...
{    
    ...
    shared_ptr<Lot> spLot = getCurrentLot();
    if(spLot)
        spLot->setModelIndex(persTestModelIndex);
    //------------------------------------------------------------------------------------
    int testIndex{};
    int slotIndex{};
    QModelIndex slotModelIndex{};
...
void MainWindow::addLotTab(const size_t lotNum, const QString& lotName)
{
    const size_t mapWidthInPixel = 1024+512;
    const size_t mapHeightInPixel = 1024+128;
...
void MainWindow::slotNormalValues()
{
    shared_ptr<TestElem> spTestElem = getCurrentTestElem();
    if(spTestElem)
        spTestElem->setTestValuesPrefix(ValuePrefix::norm);
    else return;
...
void MainWindow::slotMilliValues()
{
    shared_ptr<TestElem> spTestElem = getCurrentTestElem();
    if(spTestElem)
        spTestElem->setTestValuesPrefix(ValuePrefix::milli);
    else return;
...
void MainWindow::slotMicroValues()
{
    shared_ptr<TestElem> spTestElem = getCurrentTestElem();
    if(spTestElem)
        spTestElem->setTestValuesPrefix(ValuePrefix::micro);
    else return;
...
void MainWindow::slotNanoValues()
{
    shared_ptr<TestElem> spTestElem = getCurrentTestElem();
    if(spTestElem)
        spTestElem->setTestValuesPrefix(ValuePrefix::nano);
    else return;
...
void MainWindow::slotPicoValues()
{
    shared_ptr<TestElem> spTestElem = getCurrentTestElem();
    if(spTestElem)
        spTestElem->setTestValuesPrefix(ValuePrefix::pico);
    else return;
...
void MainWindow::slotKiloValues()
{
    shared_ptr<TestElem> spTestElem = getCurrentTestElem();
    if(spTestElem)
        spTestElem->setTestValuesPrefix(ValuePrefix::kilo);
    else return;
...
void MainWindow::slotMegaValues()
{
    shared_ptr<TestElem> spTestElem = getCurrentTestElem();
    if(spTestElem)
        spTestElem->setTestValuesPrefix(ValuePrefix::mega);
    else return;
...
void MainWindow::slotGigaValues()
{
    shared_ptr<TestElem> spTestElem = getCurrentTestElem();
    if(spTestElem)
        spTestElem->setTestValuesPrefix(ValuePrefix::giga);
    else return;
...
void MainWindow::slotTeraValues()
{
    shared_ptr<TestElem> spTestElem = getCurrentTestElem();
    if(spTestElem)
        spTestElem->setTestValuesPrefix(ValuePrefix::tera);
    else return;
...
void MainWindow::slotStatisticsRecalc(const int X, const int Y, const Mask maskState)
{
...
void MainWindow::slotSaveChangesInDies()
{
    ...
    size_t waferCount{};
    if(spLot)
        waferCount = spLot->getWaferCount();
    else return;
            ...
            if(spWafer)
                for(size_t testIndex = 0; testIndex < testElemCount; ++testIndex)
                {
                    ...
                    if(spTestElem)
                    {
                        ...
                        NormsID normsID = getNormsID();
                        testElemData.setNormsID(normsID);
                        //вычислить норму для тестового элемента
                        SystemNorms::setTestElemNorm(testElemData);
                        //установить норму в тестовый элемент
...
void MainWindow::slotShowSavingWin()
{
    ...
    if(!fileName.isEmpty())
    {
        SystemDataBase::spLot = spLot;
        SystemDataBase::createSavingWin(spLot);
        SavingWin *pSavingWin{};
        if(SystemDataBase::upSavingWin) pSavingWin = SystemDataBase::upSavingWin.get();
...
{
    SystemDataBase::pBreaker = new Breaker(this);
    SystemDataBase::pBreaker->setVisible(false);
    SystemDataBase::MainWinHeight = height();
    SystemDataBase::MainWinWidth = width();
    ...
    if(selectedSlot == -1)
        SystemDataBase::createLotDatabase(lotName);
    else
        SystemDataBase::createOneWaferTables(lotName, selectedSlot);

    if(SystemDataBase::pBreaker != nullptr)
    { delete SystemDataBase::pBreaker; SystemDataBase::pBreaker = nullptr; }
}
...
void MainWindow::slotImportDatabase()
{
    ...
    if(!fullFileName.isEmpty())
    {
        ...
        spLot->setLotNum(lotNum);
        spLot->setLotName(lotName);
        SystemDataBase::queryLotData(spLot);
        //добавление партии в группу
        addLotToGroup(spLot);
        //создание вкладки партии и дабавление ее в центральный виджет
        addLotTab(lotNum, lotName);
...
void MainWindow::slotOpenDatabaseConfig()
{
    ...
    if(!fullFileName.isEmpty())
    {
        ...
        SystemDataBase::createConfigWin(dbLotFileName_NoPathNoExt);
    }
}
void MainWindow::slotUpdateDatabase()
{
    ...
    QString strFilter{};
    QString fileName = QFileDialog::
            getOpenFileName(this, "Обновить файл базы данных ...",
                            "", "Файл базы данных (*.db)", &strFilter);
    if(!fileName.isEmpty())
    {
        SystemDataBase::pBreaker = new Breaker(this);
        ...
        SystemDataBase::MainWinHeight = height();
        SystemDataBase::MainWinWidth = width();
        SystemDataBase::updateLotDataBase(lotName);
    }
    if(SystemDataBase::pBreaker != nullptr)
    { delete SystemDataBase::pBreaker; SystemDataBase::pBreaker = nullptr; }
