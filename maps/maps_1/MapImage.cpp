...
void MapImage::drawStatisticsText(QPainter& painter, const int diesCountPainted)
{
...
//    double toLeft = m_imageSize.width()*0.02;
//    double legendSize = m_imageSize.width()*0.010;
//    QRectF legendGreen(m_LeftMargin, m_TopMargin*(FontMargin + base*(mltpl + st*0.47)),
//                       legendSize,legendSize);
//    painter.fillRect(legendGreen, colorGreen);
//    painter.drawRect(legendGreen);
//    painter.drawText(m_LeftMargin + toLeft, m_TopMargin*(FontMargin + base*(mltpl+=st)),
//                     m_txtNormalInterval);
    painter.drawText(m_LeftMargin, m_TopMargin*(FontMargin + base*(mltpl+=st)),
                     m_txtNormalInterval);

//    painter.drawText(m_LeftMargin, m_TopMargin*(FontMargin + base*(mltpl+=st)),
//                     m_txtNormalDeviation_INIT);
    //blue
//    QRectF legendBlue(m_LeftMargin, m_TopMargin*(FontMargin + base*(mltpl + st*0.47)),
//                      legendSize,legendSize);
//    painter.fillRect(legendBlue, colorBlue);
//    painter.drawRect(legendBlue);
//    painter.drawText(m_LeftMargin + toLeft, m_TopMargin*(FontMargin + base*(mltpl+=st)),
//                     m_txtNormalDeviation1);

    //red
//    QRectF legendRed(m_LeftMargin, m_TopMargin*(FontMargin + base*(mltpl + st*0.47)),
//                     legendSize,legendSize);
//    painter.fillRect(legendRed, colorRed);
//    painter.drawRect(legendRed);
//    painter.drawText(m_LeftMargin + toLeft, m_TopMargin*(FontMargin + base*(mltpl+=st)),
//                     m_txtNormalDeviation2);
    ...
    //отбражение количества нарисованных кристаллов в углу карты
    /*
    double RightMargin = m_mapSize.width() - m_LeftMargin*4;
    QString strChipCountPainted = QString::number(chipCountPainted, 10);
    painter.setFont(QFont("Times New Roman", 38));
    painter.drawText(RightMargin, m_TopMargin - m_mapSize.height()*0.01, strChipCountPainted);
    */
    // -----------------------------------------------------------------------------
    //Авторские сведения
    QString author = "Чекмарев А.В.  ", version = ", версия ";
    QString authorSignature = author + g_strYears + version + g_strVersion;
    double RightMarginAuthor = m_imageSize.width() - m_LeftMargin*22;
    painter.setPen(QPen(Qt::black, 1, Qt::SolidLine));
    painter.setFont(QFont("Times New Roman", 27));
    painter.drawText(RightMarginAuthor, m_TopMargin*(FontMargin + base*16.1), authorSignature);

    painter.end();
}
//----------------------------------------------------------------------
//устновка данных тестового элемента
void MapImage::setWaferSlot(const int waferSlot)
{
...
void MapImage::setDiesNumMeasured(const int numMeasured)
{
    m_txtDiesNumMeasured = m_txtDiesNumMeasured_INIT + QString::number(numMeasured, 10);
}
void MapImage::setDiesNumUnmasked(const int numUnmasked)
{
    m_txtUnmaskedDiesNum = m_txtUnmaskedDiesNum_INIT + QString::number(numUnmasked, 10);
}
void MapImage::setDiesNumMasked(const int numMasked)
{
    m_txtMaskedDiesNum = m_txtMaskedDiesNum_INIT + QString::number(numMasked, 10);
}
void MapImage::setDiesFit(const int diesFitNum, const double diesFitPercent)
{
...
void MapImage::setDiesUnfit(const int diesUnfitNum, const double diesUnfitPercent)
{
    m_txtUnfitDiesNum = m_txtUnFitDiesNum_INIT +  QString::number(diesUnfitNum, 10)
            + " (" + QString::number(diesUnfitPercent,'f',2) + "%)";
}

void MapImage::setAvg(const double avg)
{
...
void MapImage::setStd(const double std)
{
...
void MapImage::setMin(const double min)
{
...
void MapImage::setMax(const double max)
{
...
void MapImage::setNormsInterval(const double CriteriumDn, const double CriteriumUp)
{
    QString strCriteriumBottom;

    if( fabs(CriteriumDn) > 1.0e+3 || fabs(CriteriumDn) < 1.0e-2 )
    {
        if(CriteriumDn == 0.0)
            strCriteriumBottom = QString::number(CriteriumDn,'f',2);
        else
            strCriteriumBottom = QString::number(CriteriumDn,'e',2);
    }
    else
        strCriteriumBottom = QString::number(CriteriumDn,'f',2);
    //---------------------------------------------------------------------------
    QString strCriteriumTop;

    if( fabs(CriteriumUp) > 1.0e+3 || fabs(CriteriumUp) < 1.0e-2 )
    {
        if(CriteriumUp == 0.0)
            strCriteriumTop = QString::number(CriteriumUp,'f',2);
        else
            strCriteriumTop = QString::number(CriteriumUp,'e',2);
    }
    else
        strCriteriumTop = QString::number(CriteriumUp,'f',2);
    //---------------------------------------------------------------------------
    m_txtNormalInterval = "[" + strCriteriumBottom + "; " + strCriteriumTop + "]";
}
void MapImage::setNormsDeviations(const double CriteriumDn, const double CriteriumUp)
{
    QString strCriteriumBottom;
    if( fabs(CriteriumDn) > 1.0e+3 || fabs(CriteriumDn) < 1.0e-2 )
    {
    ...
    m_txtNormalDeviation1 = "< " + strCriteriumBottom;
    //----------------------------------------------------------------------------
    QString strCriteriumTop;
    if( fabs(CriteriumUp) > 1.0e+3 || fabs(CriteriumUp) < 1.0e-2 )
    {
    ...
    m_txtNormalDeviation2 = "> " + strCriteriumTop;
}
//-----------------------------------------------------------------------
...
void MapImage::setMaxOfMesuredDiesAlongRadius(const int MaxOfXY)
{
    m_MaxOfDiesAlongRadiusInFact = MaxOfXY;
}
void MapImage::setNumOfVoidDiesLayers(const int NumOfVoidDieLayers)
{
    m_NumOfVoidDieLayers = NumOfVoidDieLayers;
}
void MapImage::setMaxOfDiesAlongRadiusInFact(const int MaxOfDiesAlongRadius)
{
...
void MapImage::setFitDies(const vecPair& vecPairXY_Fit)
{
    if(!m_vecPairXY_Fit.isEmpty())
        m_vecPairXY_Fit.clear();

    m_vecPairXY_Fit = vecPairXY_Fit;
}
void MapImage::setUnfitDiesUpper(const vecPair& vecPairXY_UnfitMore)
{
    if(!m_vecPairXY_UnFitUpper.isEmpty())
        m_vecPairXY_UnFitUpper.clear();

    m_vecPairXY_UnFitUpper = vecPairXY_UnfitMore;
}
void MapImage::setUnfitDiesLower(const vecPair& vecPairXY_UnfitLess)
{
    if(!m_vecPairXY_UnfitLower.isEmpty())
        m_vecPairXY_UnfitLower.clear();

    m_vecPairXY_UnfitLower = vecPairXY_UnfitLess;
}
//======================================================================================
//маскирование
//запись немаскированных величин на пластину
void MapImage::setUnmaskedDies(const vecPair& vecPairXY, const QVector<double>& vecValues)
{
    if(!m_vecPairXY_Unmasked.isEmpty())
        m_vecPairXY_Unmasked.clear();

    m_vecPairXY_Unmasked = vecPairXY;

    if(!m_vecValues_Unmasked.isEmpty())
        m_vecValues_Unmasked.clear();

    m_vecValues_Unmasked = vecValues;
}
//запись маскированных величин на пластину
void MapImage::setMaskedDies(const vecPair& vecPairXY, const QVector<double>& vecValues,
                             const QVector<Mask>& vecMaskType)
{
    if(!m_vecPairXY_Masked.isEmpty())
        m_vecPairXY_Masked.clear();

    m_vecPairXY_Masked = vecPairXY;

    if(!m_vecValues_Masked.isEmpty())
        m_vecValues_Masked.clear();

    m_vecValues_Masked = vecValues;

    if(!m_vecMaskType.isEmpty())
        m_vecMaskType.clear();

    m_vecMaskType = vecMaskType;
}
//слот для приема координат int,int и состояния кристалла bool
void MapImage::slotDrawDieMasked(const int X, const int Y, const Mask maskState)
{
...
void MapImage::setScaleFactorDieSize(const double scaleFactor)
{
...
void MapImage::setMapType(const MapType mt)
{
    m_map_type = mt;
}
//==========================================================================================
