#include "Parser_4082.h"

Parser4082::Parser4082() : m_maxCountTestElem(100)
{}
//-----------------------------------------------------------------------------------
void Parser4082::doParsing(QFile &ReadFile)
{
    //-----------------------------------------------------------------------
            ...
            if(line.contains(regPos) && line.contains(regValue))
            {
                ...
                if(!valueList.isEmpty())
                {
                    ...
                    foreach (QString str, valueList)
                    {
                        ...
                        //удаление объектов-кристаллов в деструкторе главного окна
...
//-----------------------------------------------------------------------------------
void Parser4082::RegProcess(QRegExp& regInput, QString& strReg) const
{
    QStringList list1 = regInput.capturedTexts();
    foreach (QString str, list1) strReg = str;
}
shared_ptr<Lot> Parser4082::getLot() const{ return m_spLot; }
//------------------------------------------------------------------------------------
