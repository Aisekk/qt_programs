...
TestElemReport::TestElemReport(const int slotNum, const int testNum,
                               const shared_ptr<TestElem> spTestElem) :
...
{
    unique_ptr<TestElemStatistics> upTestElemStats = std::move(testElemData.getTestElemStatistics());
    TestElemStatistics *pTestElemStats{};
    if(upTestElemStats)
        pTestElemStats = upTestElemStats.get();

    m_criteriumDn = testElemData.getCriterionDn();
    m_criteriumUp = testElemData.getCriterionUp();
...
ostream& operator << (ostream& os, const TestElemReport& te)
{
    ...
    if(fabs(te.m_criteriumDn) >= restrictUp || fabs(te.m_criteriumUp) >= restrictUp
            || fabs(te.m_criteriumDn) < restrictDn || fabs(te.m_criteriumUp) < restrictDn)
        os << scientific << setprecision(p);
    else
        os << fixed << setprecision(p);

    os << " Интервал:         " << "[" << te.m_criteriumDn << ";" << te.m_criteriumUp << "]" << endl;
...
QString& operator + (QString& qs, const TestElemReport& te)
{
    ...
    qs += "Интервал:            [";
    if(fabs(te.m_criteriumDn) >= MAX || fabs(te.m_criteriumDn) < MIN
            || fabs(te.m_criteriumUp) >= MAX || fabs(te.m_criteriumUp) < MIN)
        qs += QString::number(te.m_criteriumDn,'e',2) + ";" + QString::number(te.m_criteriumUp,'e',2);
    else
        qs += QString::number(te.m_criteriumDn,'f',2) + ";" + QString::number(te.m_criteriumUp,'f',2);
    qs += "]\n";
