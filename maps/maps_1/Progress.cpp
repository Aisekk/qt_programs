...
Progress::~Progress(){}
//-------------------------------------------------------------------------------------------------
void Progress::setStep(size_t step)
{
    m_pprb->setValue(step);
}
bool Progress::isStopped() const
{
    if(m_pStopButton->isDown()) return true;
    return false;
}
void Progress::setStopButtonVisible()
{
    m_pStopButton->setVisible(true);
}
