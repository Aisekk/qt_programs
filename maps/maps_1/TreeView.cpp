...
TreeView::TreeView(QWidget* pwgt) : QTreeView(pwgt)
{
    ...
    m_pactUnmaskAllDies = new QAction("&Снять маску со всех кристаллов", nullptr);
...
TreeView::~TreeView(){}
//--------------------------------------------------------------------------------------
size_t TreeView::getLotNum() const
{ return m_lotNum; }
void TreeView::setLotNum(const size_t lotNum)
{ m_lotNum = lotNum; }

void TreeView::toHide()
{
    if(width() == 0)
        setFixedWidth(m_treeWidth);
    else
        setFixedWidth(0);
}
