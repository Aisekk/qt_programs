...
NormsID MainWindow::getNormsID() const
{
    ...
    else if(m_pactDynamicInterval->isChecked())     normsID = NormsID::Dynamic;
...
void MainWindow::slotTestElemDynIntervalWin()
{
    ...
    {
        QMessageBox::warning(this, "Ошибка", "Переключитесь на динамический интервал "
                                             "\n в меню \"Выбрать интервалы\" (СTRL+D) ");
        return;
    }
    //---------------------------------------------------------------------------------------------
...
void MainWindow::slotCalcTestElemByDynInterval(const pair<double,double>& DnUp)
{
    double dn = DnUp.first;
    double up = DnUp.second;
    auto spTestElem = getCurrentTestElem();
