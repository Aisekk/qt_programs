...
void MaskWindow::setDieButtons(QVector<shared_ptr<Die>> vec_spDies, MaskButtonType maskBtnType)
{
    ...
    std::vector< std::pair<int,int> > vecXY;
    ...
    for(std::pair<int,int>& pairXY : vecXY)
    {
...
void MaskWindow::setDieButtonToMaskedState(const int X, const int Y)
{
...
void MaskWindow::setDieButtonToUnmaskedState(const int X, const int Y)
{
    QString dieXY = QString::number(X) + "_" + QString::number(Y);
    QString toggleButton{};
