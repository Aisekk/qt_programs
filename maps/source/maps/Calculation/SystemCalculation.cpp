...
void SystemCalculation::calcTestElemStatistics(TestElemData& testElemData)
{
    ...
    if(spTestElem) testName = spTestElem->getTestName(); else return;
    //--------------------------------------------------------------------------------------
    ...
    data.criterionDn = testElemData.getCriterionDn();
    data.criterionUp = testElemData.getCriterionUp();
    pMapImage->setNormsInterval(data.criterionDn, data.criterionUp);
    pMapImage->setNormsDeviations(data.criterionDn, data.criterionUp);
    //-------------------------------------------------------------------------------------------
...
void SystemCalculation::StatisticsCalc(const CalcData& in, Statistica& out)
{
    ...
    for(int k = 0; k < in.diesNumUnmasked; ++k)
        if(in.vecValues_Unmasked[k] >= in.criterionDn &&
                in.vecValues_Unmasked[k] <= in.criterionUp)
        {
            ++out.diesNumFit;
            if( (in.criterionDn >= 0 && in.criterionUp >= 0) ||
                    (in.criterionDn <= 0 && in.criterionUp <= 0))
                out.avg += in.vecValues_Unmasked[k];
            else if(in.criterionDn < 0 && in.criterionUp > 0)
        ...
        else if(in.vecValues_Unmasked[k] < in.criterionDn)
            out.vecPairXY_UnfitLower.push_back(in.vecPairXY_Unmasked[k]);
        else if(in.vecValues_Unmasked[k] > in.criterionUp)
    ...
    if(out.diesNumFit != 0 && out.avg != 0.0)
    {
        for(int k = 0; k < in.vecValues_Unmasked.size(); ++k)
            if(in.vecValues_Unmasked[k] > in.criterionDn && in.vecValues_Unmasked[k] < in.criterionUp)
            {
                if( (in.criterionDn >= 0 && in.criterionUp >= 0) ||
                        (in.criterionDn <= 0 && in.criterionUp <= 0) )
                                            ...
                                            const ValuePrefix valuePrefix)
{
    ...
    QRegExp tRZ("tRZ"), KLV("KLV"), dM("dM"), VDP("VDP"), R("R"), Rs("Rs");
    QString Res = "Ом";
    if(fullName.contains(pNpN) || fullName.contains(tRZ) || fullName.contains(KLV)
            || fullName.contains(dM) || fullName.contains(VDP)
            || paramName.contains(R) || paramName.contains(Rs))
    { Unit += Res; return Unit; }
    ...
    { Unit += Volt; return Unit; }
    //напряжение 2
    QString UGST("UGST"), UGST_01("UGST_01"), UBR("UBR"), VBR("VBR");
    if(paramName.contains(UGST) || paramName.contains(UGST_01)
            || paramName.contains(UBR) || paramName.contains(VBR))
    { Unit += Volt; return Unit; }
    ...
    if((fullName.contains(Ap) || (fullName.contains(An))
        ...
        || (fullName.contains(Hp)) || (fullName.contains(Hn)))
            && (fullName.contains(DSw_leak)
                || (fullName.contains(DS) && fullName.contains(leak) && fullName.contains(w))))
    { Unit += AmperPerMikron; return Unit; }
    ...
    //приведенный ток
    QString ID("ID"), IDSS("IDSS");
    if(paramName.contains(ID) || paramName.contains(IDSS))
    { Unit += AmperPerMikron; return Unit; }

    //крутизна
    QString Be("Be"), be("be"), AdV2("А/В2");
    if(fullName.contains(Be) || fullName.contains(be)
            || paramName.contains(Be) || paramName.contains(be))
    { Unit += AdV2; return Unit; }
